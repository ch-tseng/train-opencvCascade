...
negativeFolder = "neg-images"  # all negative images will save to this folder
negativeDP = "neg-ml"  # thhe size is same with roi size, will be used for ML
negativeSize = (128,128)
negativeType = "jpg"
maxImageUsed = 50  # How many negative images will be used (from background and neg source images)
negativeImageFolder = "datasets/bgSources"  # source for more negative images, except the background images.
...
negativeDP = folderPROJECT + "/" + negativeDP
...
if(makeNegativeDataset==True):
    ...
    if not os.path.exists(negativeDP + "/"):
        os.makedirs(negativeDP + "/")
