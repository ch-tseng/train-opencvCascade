...
def createPositive(imgFolder, xmlFilename, descFile, assignName, assignName2):
...
#    with open(descFile, 'a') as the_file_aug:
    for i in range(0, len(labelName)):
        if(assignName=="" or assignName==labelName[i] or assignName2==labelName[i]):
            countLabels += 1
...
with open(positiveDesc, 'a') as the_file:
    ...
    for file in os.listdir(xmlFolder):
        ...
        if(file_extension==".xml"):
            ...
            outLabels = createPositive(imgFolder, xmlfile, positiveDesc_aug, labelName, labelName2)
